...
{
    public class CreateVariableXByInstituteCommandHandler
        : IRequestHandler<CreateVariableXByInstituteCommand, ErrorOr<VariableResultX>>
    {
        private readonly IVariableXRepository _repository;

        public CreateVariableXByInstituteCommandHandler(IVariableXRepository repository)
        {
            _repository = repository;
        }

        public async Task<ErrorOr<VariableResultX>> Handle(CreateVariableXByInstituteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (string.Compare(groupName, "@IES", true) == 0)
            {
