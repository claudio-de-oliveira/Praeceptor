...
public class GetPeaTextByClassIdQueryHandler
    : IRequestHandler<GetPeaTextByClassIdQuery, ErrorOr<PlannerTextResult>>
{
    private readonly IClassRepository _classRepository;
    private readonly IPeaRepository _peaRepository;

    public GetPeaTextByClassIdQueryHandler(IPeaRepository peaRepository, IClassRepository classRepository)
    {
        _peaRepository = peaRepository;
        _classRepository = classRepository;
    }

    public async Task<ErrorOr<PlannerTextResult>> Handle(GetPeaTextByClassIdQuery request, CancellationToken cancellationToken)
    {
        if (cancellationToken.IsCancellationRequested)
            return Domain.Errors.Error.Pea.Canceled;

        var pea = await _peaRepository.GetPeaByClassId(request.ClassId);
        if (pea is null)
