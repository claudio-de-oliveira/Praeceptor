...
namespace Word
{
    ...
    {
        enum CaptionType { FIGURE, TABLE, EQUATION };
        ...
        public override void Execute(AbstractTAG action, Stack<AbstractTAG> stk, Stack<AbstractToken> tokens, AbstractEnvironment<WordDocument> environment)
        {
            try
            {
                ...
                if (action == Tag._IncludeTable)
                {
                    ...
                    var table = ((PPCModel)((WordDocEnvironment)environment).WordDocument).GetTableByCode(1, key).GetAwaiter().GetResult();

                    if (table is not null && !string.IsNullOrWhiteSpace(table.Text))
                    {
                        ...
                        if (!result)
                        {
                            ((PPCModel)((WordDocEnvironment)environment).WordDocument).CreateFromText(
                ...
                if (action == Tag._Includegraphics)
                {
                    ...
                    try
                    {
                        ...
                        var instituteId = ((PPCModel)((WordDocEnvironment)environment).WordDocument).InstituteId;


                        ImageModel image = ((WordDocEnvironment)environment).WordDocument.GetImageByCode(instituteId, imageKey).GetAwaiter().GetResult();

                        if (image is null)
                        {
                            // Console.WriteLine($"---> Não consegui ler imagem \"{imageKey}\"");
                            image = ((WordDocEnvironment)environment).WordDocument.GetImageByCode(instituteId, "NotFound").GetAwaiter().GetResult();
                        }

                        if (image is not null)
                        {
                            byte[] buffer = image.Data;// Convert.FromBase64String(image.VectorImage["data:image/gif;base64,".Length..]);
