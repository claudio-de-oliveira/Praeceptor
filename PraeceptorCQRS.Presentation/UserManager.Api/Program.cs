...
var Audience = builder.Configuration.GetSection("UserManager.API").GetSection("Audience").Value;
...
builder.Services.AddAuthorization(options =>
{
    options.AddPolicy("ReadPolice", policy =>
          policy.RequireClaim("scope", "UserManager.API.Read", "UserManager.API.FullAccess"));
    options.AddPolicy("CreatePolice", policy =>
          policy.RequireClaim("scope", "UserManager.API.Create", "UserManager.API.FullAccess"));
    options.AddPolicy("UpdatePolice", policy =>
          policy.RequireClaim("scope", "UserManager.API.Update", "UserManager.API.FullAccess"));
    options.AddPolicy("DeletePolice", policy =>
          policy.RequireClaim("scope", "UserManager.API.Delete", "UserManager.API.FullAccess"));
});
#endregion
